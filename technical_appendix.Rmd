...
The following chunk calculates the percentage change, from the earlier to latter period, in ASMR by sex and SIMD quintile. 
...
The following shows how the code chunk above can be adapted to show the absolute changes as a figure.
...
The following table now includes the relative changes, followed by the absolute changes as a separate row

```{r}
tbl_both %>%
  gather(key = "key", value = "value", -sex, -period) %>%
  ...
  select(sex, period, `Q.1 (Most deprived)`:`Overall`, everything()) %>% 
    gather(key = "key", value = "value", -sex, -period) %>% 
    mutate(measure = "Absolute")
  ) %>% 
  mutate(period = factor(period, levels = c("2006-2011", "2012-2017")))  %>%
  spread(key, value) %>% 
  select(sex, period, measure, `Q.1 (Most deprived)`:`Q.5 (Least deprived)`, Overall, everything()) %>% 
  arrange(sex, period, measure) %>% 
  knitr::kable(
    digits = 2, 
    caption = "Percent change in ASMR by sex, SIMD quintile, and period. (Alternative method)"
  ) %>% 
